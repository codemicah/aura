...
export function RiskAssessment({ onComplete, className = '' }: RiskAssessmentProps) {
  const { calculateRiskScore, isLoading: isCalculating, error: apiError } = useRiskAssessment()
  ...
  const steps = [
    ...
    },
    {
      ...
      options: [
        { value: 'short_term', label: 'Short-term savings (< 2 years)', points: 5 },
        { value: 'medium_term', label: 'Medium-term goals (2-5 years)', points: 15 },
        { value: 'long_term', label: 'Long-term growth (5-10 years)', points: 20 },
        { value: 'retirement', label: 'Retirement planning (10+ years)', points: 25 }
      ]
    },
    {
      ...
      options: [
        { value: 'very_low', label: 'Very Conservative - Safety is my top priority', points: 5 },
        { value: 'low', label: 'Conservative - I prefer stable, lower returns', points: 10 },
        { value: 'medium', label: 'Balanced - I want moderate risk and returns', points: 15 },
        { value: 'high', label: 'Growth-oriented - Higher risk for better returns', points: 20 },
        { value: 'very_high', label: 'Aggressive - Maximum growth potential', points: 25 }
      ]
    },
    {
    ...
    },
    {
      ...
      options: [
        ...
        { value: 'medium', label: 'Might need some funds occasionally', points: 15 },
        { value: 'low', label: 'Won\'t need these funds for a long time', points: 25 }
      ]
    }
  ]
  ...
  const handleComplete = async (lastValue: string, lastPoints: number) => {
    ...
    const apiAnswers: RiskAssessmentAnswers = {
      age: allSelections.age === '18-25' ? 22 : 
           allSelections.age === '26-35' ? 30 :
           allSelections.age === '36-50' ? 43 : 55,
      income: allSelections.income === 'under-40k' ? 35000 :
              allSelections.income === '40k-80k' ? 60000 :
              allSelections.income === '80k-150k' ? 115000 : 200000,
      monthlyExpenses: allSelections.expenses === 'under-2k' ? 1500 :
                       allSelections.expenses === '2k-4k' ? 3000 :
                       allSelections.expenses === '4k-6k' ? 5000 : 7500,
    ...
    try {
      ...
      const riskProfile = riskScore <= 33 ? 'Conservative' : riskScore <= 66 ? 'Balanced' : 'Aggressive'
        ...
        <div className="mt-6 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-lg">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-blue-900">
              Based on your answers, you'll likely have a {getRiskLevel(Math.round((currentStep + 1) * 11.11))} risk profile
            </span>
          </div>
        </div>
      )}
