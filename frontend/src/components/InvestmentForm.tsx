...
export function InvestmentForm({ className = "" }: InvestmentFormProps) {
  ...
  const { riskScore = 50 } = useRiskProfile();
  ...
  const handleInvest = async () => {
    ...
    if (balance && parseFloat(amount) > parseFloat(balance.formatted)) {
      notifyError(
        ...
        "You don't have enough AVAX for this investment."
      );
      return;
    }
    ...
    try {
      ...
      notifyInfo(
        "Transaction Started",
        "Your investment transaction has been initiated. Please check your wallet."
      );
    } catch (error) {
  ...
  const handleRebalance = async () => {
    try {
      ...
      notifyInfo(
        "Rebalance Started",
        "Portfolio rebalancing has been initiated."
      );
    } catch (error) {
  ...
  const handleWithdraw = async () => {
    if (
      window.confirm(
        "Are you sure you want to withdraw all funds? This will close all your positions."
      )
    ) {
      try {
        await withdraw();
        notifyWarning(
          "Withdrawal Started",
          "Emergency withdrawal has been initiated. All positions will be closed."
        );
      } catch (error) {
  ...
  const getRiskLevelColor = (score: number) => {
    ...
    if (score <= CONTRACT_CONSTANTS.RISK_RANGES.BALANCED.max)
    ...
    return (
      ...
      >
        ...
        <div className="text-center py-8">
          ...
          <p className="text-gray-600 text-sm">
            Switch to Avalanche or Fuji testnet
          </p>
        </div>
      </div>
    );
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <button
                  ...
                  disabled={
                    isLoadingBalance || isOptimizing || isOptimizeConfirming
                  }
                >
                  MAX
                </button>
            ...
            <div>
              ...
              <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                <span
                  className={`font-medium ${getRiskLevelColor(riskScore!)}`}
                >
            ...
            <button
              ...
              disabled={
                !amount ||
                parseFloat(amount) <
                  parseFloat(CONTRACT_CONSTANTS.MIN_DEPOSIT) ||
                isOptimizing ||
                isOptimizeConfirming ||
                isLoadingBalance
              }
          ...
          {!portfolioMetrics || portfolioMetrics.totalDeposited === 0 ? (
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 text-center">
              ...
              <p className="text-gray-500 text-xs">
                Invest funds first to enable portfolio actions
              </p>
            </div>
          ) : (
            <div className="space-y-3">
              ...
              <div className="bg-gray-50 rounded-lg p-3 mb-3">
                <div className="flex justify-between items-center text-sm">
                  ...
                  <span className="font-medium text-gray-900">
                    {portfolioMetrics.totalDeposited.toFixed(4)} AVAX
                  </span>
                </div>
                <div className="flex justify-between items-center text-sm mt-1">
                  <span className="text-gray-600">Current Value:</span>
                  <span className="font-medium text-gray-900">
                    {portfolioMetrics.totalValue.toFixed(4)} AVAX
                  </span>
                </div>
                {portfolioMetrics.daysSinceLastRebalance > 0 && (
                  <div className="text-xs text-gray-500 mt-2">
                    Last rebalanced: {portfolioMetrics.daysSinceLastRebalance}{" "}
                    day
                    {portfolioMetrics.daysSinceLastRebalance !== 1
                      ? "s"
                      : ""}{" "}
                    ago
                  </div>
              ...
              <button
                ...
                disabled={
                  ...
                  isRebalanceConfirming ||
                  !portfolioMetrics ||
                  portfolioMetrics.totalDeposited === 0
                }
                ...
                title={
                  !portfolioMetrics || portfolioMetrics.totalDeposited === 0
                    ? "Invest funds first to enable rebalancing"
                    : "Rebalance your portfolio to optimal allocations"
                }
              >
              ...
              <button
                ...
                title={
                  !portfolioMetrics || portfolioMetrics.totalDeposited === 0
                    ...
                    : "Withdraw all funds from positions"
                }
              >
