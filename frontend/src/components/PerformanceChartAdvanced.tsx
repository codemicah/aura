...
export default function PerformanceChartAdvanced({
  ...
  const benqiAmount = portfolio?.allocation?.benqiAmount || '0';
  ...
  const { data, stats } = useMemo(() => {
    ...
    for (let i = days; i >= 0; i--) {
      ...
      newData.push({
        ...
        benqi: Math.round(currentValue * benqiAllocation * 100) / 100,
        traderjoe: Math.round(currentValue * traderjoeAllocation * 100) / 100,
        yieldyak: Math.round(currentValue * yieldyakAllocation * 100) / 100,
        benchmark: Math.round(baseValue * Math.pow(1.001, days - i) * 100) / 100, // 0.1% daily benchmark
      });
    }
  ...
  }, [selectedPeriod, totalValueStr, benqiAmount, traderJoeAmount, yieldYakAmount, totalEarnings, returnPercentage, avaxPrice]);

  // Show loading state if price is not available - MUST come after all hooks
  if (isPriceLoading || !avaxPrice) {
    return (
      <div className="bg-gray-900 rounded-xl border border-gray-800 p-6" style={{ height }}>
      ...
      <div className="bg-gray-800 border border-gray-600 rounded-lg p-3 shadow-lg">
        ...
        <div className="space-y-1">
          <div className="flex items-center justify-between space-x-4">
            ...
            <span className="text-white font-medium">${payload[0]?.value?.toFixed(2)}</span>
          </div>
          {showBenchmark && payload[4] && (
            <div className="flex items-center justify-between space-x-4">
              <span className="text-gray-400 text-sm">Benchmark:</span>
              <span className="text-gray-300">${payload[4]?.value?.toFixed(2)}</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 space-y-4 sm:space-y-0">
        <div>
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className={`flex items-center space-x-1 ${stats.isPositive ? 'text-green-400' : 'text-red-400'}`}>
              {stats.isPositive ? (
              ...
              <span className="font-medium">
                {stats.isPositive ? '+' : ''}{stats.percentReturn.toFixed(2)}%
              </span>
              <span className="text-gray-400 text-sm">
                ({stats.isPositive ? '+' : ''}${Math.abs(stats.totalReturn).toFixed(2)})
              </span>
            </div>
          </div>
        </div>
      ...
      <ResponsiveContainer width="100%" height={height}>
        <ComposedChart data={data} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
          <defs>
            ...
            <linearGradient id="benqiGradient" x1="0" y1="0" x2="0" y2="1">
      ...
      <div className="mt-6 grid grid-cols-3 gap-4">
        <div className="bg-gray-700 rounded-lg p-3">
          <div className="flex items-center justify-between mb-1">
            <span className="text-gray-400 text-sm">Benqi</span>
        ...
        <div className="bg-gray-700 rounded-lg p-3">
          ...
          <div className="w-full bg-gray-600 rounded-full h-2">
            <div className="bg-amber-400 h-2 rounded-full" style={{ width: '40%' }} />
          </div>
        </div>
