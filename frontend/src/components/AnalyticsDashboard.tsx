...
const AnalyticsDashboard: React.FC<AnalyticsDashboardProps> = ({ userId }) => {
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "protocols" | "risk" | "benchmarks"
  >("overview");
  ...
  const calculatedMetrics = useMemo(() => {
    ...
    const totalDeposited =
      parseFloat(portfolio.profile?.totalDeposited || "0") * avaxPrice;
    ...
    if (history && history.length > 1) {
      ...
      const weekAgoValue =
        history[Math.min(7, history.length - 1)]?.totalValue || totalValue;
      weeklyReturn =
        weekAgoValue > 0
          ? ((todayValue - weekAgoValue) / weekAgoValue) * 100
          : 0;

      // Calculate monthly return (30 days ago)
      const monthAgoValue =
        history[Math.min(30, history.length - 1)]?.totalValue || totalValue;
      monthlyReturn =
        monthAgoValue > 0
          ? ((todayValue - monthAgoValue) / monthAgoValue) * 100
          : 0;
  ...
  const protocols = useMemo<ProtocolPerformance[]>(() => {
    if (portfolio && yields && avaxPrice) {
      ...
      const aaveValue =
        parseFloat(portfolio.allocation?.aaveAmount || "0") * avaxPrice;
      const traderJoeValue =
        parseFloat(portfolio.allocation?.traderJoeAmount || "0") * avaxPrice;
      const yieldYakValue =
        parseFloat(portfolio.allocation?.yieldYakAmount || "0") * avaxPrice;

      // Calculate deposited amounts (assuming 20% return on average)
      const avgReturn = 0.2;
      const aaveDeposited = aaveValue / (1 + avgReturn);
      ...
      const totalProtocolValue = aaveValue + traderJoeValue + yieldYakValue;
      const aaveAllocation =
        totalProtocolValue > 0 ? (aaveValue / totalProtocolValue) * 100 : 0;
      const traderJoeAllocation =
        totalProtocolValue > 0
          ? (traderJoeValue / totalProtocolValue) * 100
          : 0;
      const yieldYakAllocation =
        totalProtocolValue > 0 ? (yieldYakValue / totalProtocolValue) * 100 : 0;

      return [
        {
          protocol: "Aave",
          currentValue: aaveValue,
          totalDeposited: aaveDeposited,
          totalReturn: aaveValue - aaveDeposited,
          returnPercentage:
            aaveDeposited > 0
              ? ((aaveValue - aaveDeposited) / aaveDeposited) * 100
              : 0,
          currentAPY: yields?.find((y) => y.protocol === "aave")?.apy || 7.5,
          averageAPY: yields?.find((y) => y.protocol === "aave")?.apy || 7.2,
        ...
        },
        {
          ...
          returnPercentage:
            traderJoeDeposited > 0
              ? ((traderJoeValue - traderJoeDeposited) / traderJoeDeposited) *
                100
              : 0,
          currentAPY:
            yields?.find((y) => y.protocol === "traderjoe")?.apy || 11.2,
          averageAPY:
            yields?.find((y) => y.protocol === "traderjoe")?.apy || 10.8,
        ...
        },
        {
          ...
          returnPercentage:
            yieldYakDeposited > 0
              ? ((yieldYakValue - yieldYakDeposited) / yieldYakDeposited) * 100
              : 0,
          currentAPY:
            yields?.find((y) => y.protocol === "yieldyak")?.apy || 15.8,
          averageAPY:
            yields?.find((y) => y.protocol === "yieldyak")?.apy || 15.2,
      ...
      return [
        {
          ...
          currentAPY: yields?.find((y) => y.protocol === "aave")?.apy || 7.5,
          averageAPY: yields?.find((y) => y.protocol === "aave")?.apy || 7.2,
          allocation: 40, // Default 40% allocation for Aave
          gasFeesSpent: 45.2,
        },
        {
          protocol: "TraderJoe",
  ...
  const riskMetricsData = useMemo(() => {
    ...
    return {
      ...
      calmarRatio:
        calculatedMetrics.maxDrawdown !== 0
          ? Math.abs(
              calculatedMetrics.annualizedReturn / calculatedMetrics.maxDrawdown
            )
          : 0,
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex space-x-2">
          {(["overview", "protocols", "risk", "benchmarks"] as const).map(
            (tab) => (
              <button
                key={tab}
      ...
      {calculatedMetrics && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <MetricCard
            ...
            subtitle={`${calculatedMetrics.totalReturnPercentage.toFixed(
              2
            )}% gain`}
          />
          <MetricCard
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="bg-gray-900 rounded-lg p-4">
                  ...
                  <p
                    className={`text-xl font-bold ${
                      calculatedMetrics.dailyReturn >= 0
                        ? "text-green-400"
                        : "text-red-400"
                    }`}
                  >
                ...
                <div className="bg-gray-900 rounded-lg p-4">
                  ...
                  <p
                    className={`text-xl font-bold ${
                      calculatedMetrics.weeklyReturn >= 0
                        ? "text-green-400"
                        : "text-red-400"
                    }`}
                  >
                    {calculatedMetrics.weeklyReturn >= 0 ? "+" : ""}
                    {calculatedMetrics.weeklyReturn.toFixed(2)}%
                  </p>
                </div>
                <div className="bg-gray-900 rounded-lg p-4">
                  ...
                  <p
                    className={`text-xl font-bold ${
                      calculatedMetrics.monthlyReturn >= 0
                        ? "text-green-400"
                        : "text-red-400"
                    }`}
                  >
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <p className="text-yellow-400 font-semibold">
                    {calculatedMetrics.volatility.toFixed(2)}%
                  </p>
                </div>
                <div>
                  <p className="text-gray-400 text-sm">Best Day</p>
                  <p className="text-green-400 font-semibold">
                    +{calculatedMetrics.bestDay.return.toFixed(2)}%
                  </p>
                </div>
                <div>
                  <p className="text-gray-400 text-sm">Worst Day</p>
                  <p className="text-red-400 font-semibold">
                    {calculatedMetrics.worstDay.return.toFixed(2)}%
                  </p>
                </div>
                <div>
                  <p className="text-gray-400 text-sm">Total Deposited</p>
                  <p className="text-white font-semibold">
                    ${calculatedMetrics.totalDeposited.toLocaleString()}
                  </p>
                </div>
                <div>
                    ...
                    >
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <span className="text-green-400">
                          +{protocol.returnPercentage.toFixed(1)}%
                        </span>
                      </div>
                      <div className="text-sm space-y-1">
                        <div className="flex justify-between">
                          ...
                          <span className="text-white">
                            ${protocol.currentValue.toLocaleString()}
                          </span>
                        </div>
                        <div className="flex justify-between">
                        ...
                        <div className="flex justify-between">
                          ...
                          <span className="text-yellow-400">
                            ${protocol.gasFeesSpent.toFixed(2)}
                          </span>
                        </div>
                      </div>
                    </div>
                  ...
                  {benchmarks.map((benchmark, index) => (
                    ...
                    >
                      ...
                      <td className="text-right py-3">
                        ...
                        >
                          {benchmark.totalReturn >= 0 ? "+" : ""}
                          {benchmark.totalReturn.toFixed(2)}%
                        </span>
                      </td>
                      <td className="text-right py-3">
                        <span className="text-white">
                      ...
                      <td className="text-right py-3">
                        <span className="text-red-400">
                          {benchmark.maxDrawdown.toFixed(2)}%
                        </span>
                      </td>
                      <td className="text-right py-3">
                        {benchmark.outperformance !== 0 && (
                          ...
                          >
                            {benchmark.outperformance > 0 ? "+" : ""}
                            {benchmark.outperformance.toFixed(1)}%
                          </span>
                        )}
                      </td>
                    </tr>
        ...
        <div className="flex items-start space-x-3">
          ...
          <div>
            ...
            <p className="text-gray-300 text-sm">
              Your portfolio is performing well with a Sharpe ratio above 1.5,
              indicating strong risk-adjusted returns. Consider monitoring the
              volatility levels and rebalancing when allocation drifts exceed
              10%.
            </p>
          </div>
        </div>
