...
import { SkeletonPortfolioOverview, SkeletonInvestmentForm } from '../../src/components/SkeletonLoaders';
import { useYieldOptimizer } from '../../src/hooks/useYieldOptimizer';
import { usePortfolio, useTransactionHistory } from '../../src/hooks/usePortfolio';
...
function DashboardContent() {
  const { yields, isLoadingYields, portfolio: yieldOptimizerPortfolio, portfolioMetrics } = useYieldOptimizer();
  ...
  const { hasProfile, isLoading: profileLoading, riskScore, riskProfile } = useRiskProfile();
  const { isConnected } = useAccount();
  const [isLoading, setIsLoading] = useState(true);
  const [activeTab, setActiveTab] = useState<'overview' | 'invest' | 'history'>('overview');
    ...
    <div className="min-h-screen bg-gray-900">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-400">Monitor and manage your DeFi investments</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <StatCard
            ...
            value={`$${((parseFloat(yieldOptimizerPortfolio?.totalValue || '0') * (avaxPrice || 0))).toFixed(2)}`}
            change={portfolioMetrics?.returnPercentage ? `${portfolioMetrics.returnPercentage >= 0 ? '+' : ''}${portfolioMetrics.returnPercentage.toFixed(2)}%` : undefined}
            icon={DollarSign}
            color="bg-gradient-to-br from-blue-600 to-blue-700"
          />
          <StatCard
            title="Average Yield (APY)"
            value={yields ? `${((yields.benqi + yields.traderJoe + yields.yieldYak) / 3).toFixed(1)}%` : '0%'}
            change={undefined}
            icon={Percent}
          ...
          <StatCard
            ...
            value={(() => {
              ...
              if (yieldOptimizerPortfolio?.allocation) {
                if (parseFloat(yieldOptimizerPortfolio.allocation.benqiAmount) > 0) count++;
                if (parseFloat(yieldOptimizerPortfolio.allocation.traderJoeAmount) > 0) count++;
                if (parseFloat(yieldOptimizerPortfolio.allocation.yieldYakAmount) > 0) count++;
              }
                      ...
                      <div key={index} className="flex items-center justify-between py-4 border-b border-gray-700 last:border-0">
                        <div className="flex items-center space-x-4">
                          ...
                          <div>
                            ...
                            <p className="text-gray-400 text-sm">{tx.timestamp?.toLocaleDateString() || 'Today'}</p>
                          </div>
                        </div>
                        <div className="text-right">
                          <p className="text-white font-medium">{tx.amount} AVAX</p>
                ...
                <div className="space-y-3">
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-gray-400 text-sm">Benqi APY</span>
                      <span className="text-green-400 font-medium">
                        {yields?.benqi?.toFixed(1) || '5.2'}%
                      </span>
                    </div>
                    <div className="w-full bg-gray-700 rounded-full h-1.5">
                      <div className="bg-green-400 h-1.5 rounded-full" style={{ width: '52%' }} />
                    </div>
                  </div>
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-gray-400 text-sm">TraderJoe APY</span>
                      <span className="text-blue-400 font-medium">
                        {yields?.traderJoe?.toFixed(1) || '8.7'}%
                      </span>
                    </div>
                    <div className="w-full bg-gray-700 rounded-full h-1.5">
                      <div className="bg-blue-400 h-1.5 rounded-full" style={{ width: '87%' }} />
                    </div>
                  </div>
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-gray-400 text-sm">YieldYak APY</span>
              ...
              <div className="flex items-start space-x-3">
                ...
                <div>
                  ...
                  <p className="text-gray-300 text-sm">
                    Enable auto-rebalancing to maximize your yields across protocols automatically.
                  </p>
