...
import { TrendingUp, Shield, Target, Edit2, CheckCircle, AlertCircle, Info } from 'lucide-react'
...
function RiskProfileContent() {
  ...
  const getRiskLevelDescription = (score: number) => {
    if (score <= 33) return 'Your strategy focuses on capital preservation with stable, lower-risk yields'
    if (score <= 66) return 'Your strategy balances growth and stability with moderate risk exposure'
    return 'Your strategy maximizes growth potential with higher risk tolerance'
  }

  const getAllocationStrategy = (score: number) => {
    if (score <= 33) return { benqi: 70, traderjoe: 30, yieldyak: 0 }
    if (score <= 66) return { benqi: 40, traderjoe: 40, yieldyak: 20 }
        ...
        <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-24">
          <div className="text-center mb-12">
            <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">
              {profileData ? 'Update Your Risk Profile' : 'Create Your Risk Profile'}
            </h1>
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-24">
        ...
        <div className="mb-8">
          ...
          <p className="text-xl text-gray-600">
            Your personalized investment strategy based on your financial goals and risk tolerance
          </p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 mb-8">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
            <div>
              <div className={`inline-flex items-center px-4 py-2 rounded-full text-sm font-semibold border ${getRiskLevelColor(currentScore)} mb-4`}>
            ...
            <div className="overflow-hidden h-4 mb-4 text-xs flex rounded-full bg-gray-200">
              <div 
                ...
                className={`shadow-none flex flex-col text-center whitespace-nowrap text-white justify-center transition-all duration-500 ${
                  currentScore <= 33 ? 'bg-green-500' : 
                  currentScore <= 66 ? 'bg-blue-500' : 'bg-purple-500'
                }`}
              />
            </div>
          </div>
              ...
              <div key={protocol.name} className="bg-gray-50 rounded-xl p-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className={`text-2xl font-bold ${
                    protocol.percentage > 0 ? 'text-gray-900' : 'text-gray-400'
                  }`}>
                    {protocol.percentage}%
                  </span>
                </div>
                <h4 className="font-semibold text-gray-900 mb-1">{protocol.name}</h4>
                <p className="text-sm text-gray-600 mb-2">{protocol.description}</p>
                <div className="text-xs font-medium text-green-600">APY: {protocol.apy}</div>
            ...
            <div className="flex items-start gap-2">
              ...
              <div>
                <p className="text-sm font-medium text-blue-900">
                  This allocation is optimized for your {getRiskLevelName(currentScore).toLowerCase()} risk profile
                </p>
                <p className="text-sm text-blue-700 mt-1">
                  The strategy automatically rebalances based on market conditions and protocol performance
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center py-3 border-b border-gray-100">
                ...
                <span className="text-xl font-bold text-gray-900">{returns.min}% APY</span>
              </div>
              <div className="flex justify-between items-center py-3 border-b border-gray-100">
                <span className="text-gray-600">Average Expected</span>
                <span className="text-2xl font-bold text-green-600">{returns.avg}% APY</span>
              </div>
              <div className="flex justify-between items-center py-3">
                <span className="text-gray-600">Best Case Scenario</span>
                <span className="text-xl font-bold text-gray-900">{returns.max}% APY</span>
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
            ...
            <div className="space-y-3">
              {profileData && [
                ...
                { label: 'Investment Goal', value: profileData.goal?.replace('_', ' ') },
                { label: 'Time Horizon', value: `${profileData.timeHorizon} years` },
