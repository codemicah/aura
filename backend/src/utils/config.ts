...
export const config = {
  ...
  TRADERJOE_ROUTER_ADDRESS:
    process.env.TRADERJOE_ROUTER_ADDRESS ||
    "0x0000000000000000000000000000000000000000",
  BENQI_COMPTROLLER_ADDRESS:
    process.env.BENQI_COMPTROLLER_ADDRESS ||
    "0x0000000000000000000000000000000000000000",
  YIELDYAK_FARM_ADDRESS:
    process.env.YIELDYAK_FARM_ADDRESS ||
    "0x0000000000000000000000000000000000000000",
  ...
  MIN_PORTFOLIO_VALUE_USD: parseFloat(
    process.env.MIN_PORTFOLIO_VALUE_USD || "100"
  ),
  REBALANCE_THRESHOLD_PERCENT: parseFloat(
    process.env.REBALANCE_THRESHOLD_PERCENT || "5"
  ),
...
export const validateConfig = (): void => {
  ...
  if (config.isProduction) {
    const prodRequired = ["JWT_SECRET", "YIELD_OPTIMIZER_ADDRESS"];

    const missingProd = prodRequired.filter(
      (key) =>
        !process.env[key] ||
        process.env[key] === "default-secret-change-in-production"
    );
