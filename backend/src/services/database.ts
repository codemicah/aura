...
export class DatabaseService {
  ...
  async createUserProfile(data: Omit<UserProfile, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> {
  ...
  async updateUserProfile(address: string, updates: Partial<UserProfile>): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      const sql = `UPDATE user_profiles SET ${setClause.join(', ')} WHERE address = ?`
  ...
  async updateTransactionStatus(hash: string, status: Transaction['status'], blockNumber?: number): Promise<void> {
    return new Promise((resolve, reject) => {
      if (!this.db) return reject(new APIError('Database not connected'))

      const sql = 'UPDATE transactions SET status = ?, block_number = ? WHERE hash = ?'
