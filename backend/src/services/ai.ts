...
export class AIService {
  ...
  calculateRiskScore(answers: RiskAssessmentAnswers): number {
    ...
    score += experienceScores[answers.investmentExperience] * weights.experience
  ...
  async generateAllocationStrategy(
    ...
    try {
      ...
      const marketData = currentMarketData || await defiDataService.getAllProtocolData()
      ...
      if (benqiData && traderJoeData && yieldYakData) {
        ...
        const avgYield = (benqiData.apy + traderJoeData.apy + yieldYakData.apy) / 3
        
        const benqiAdjustment = ((benqiData.apy - avgYield) / avgYield) * 10
        const traderJoeAdjustment = ((traderJoeData.apy - avgYield) / avgYield) * 10
        const yieldYakAdjustment = ((yieldYakData.apy - avgYield) / avgYield) * 10

        allocation.benqi = Math.max(5, Math.min(85, allocation.benqi + benqiAdjustment))
        allocation.traderJoe = Math.max(5, Math.min(85, allocation.traderJoe + traderJoeAdjustment))
        allocation.yieldYak = Math.max(0, Math.min(80, allocation.yieldYak + yieldYakAdjustment))

        // Normalize to 100%
        const total = allocation.benqi + allocation.traderJoe + allocation.yieldYak
      ...
      // Generate rationale
      const rationale = this.generateAllocationRationale(riskProfile, allocation, marketData)
      ...
      const defaultAllocations = {
        Conservative: { benqi: 70, traderJoe: 30, yieldYak: 0, expectedAPY: 0.065, riskLevel: 'low' as const },
        Balanced: { benqi: 40, traderJoe: 40, yieldYak: 20, expectedAPY: 0.088, riskLevel: 'medium' as const },
        Aggressive: { benqi: 20, traderJoe: 30, yieldYak: 50, expectedAPY: 0.112, riskLevel: 'high' as const }
      }
  ...
  private generateAllocationRationale(
    ...
    const traderJoeAPY = marketData.find(p => p.protocol === 'traderjoe')?.apy || 8.7
    const yieldYakAPY = marketData.find(p => p.protocol === 'yieldyak')?.apy || 12.4

    let rationale = `This ${riskProfile} allocation strategy `

    if (allocation.benqi >= 50) {
      rationale += `prioritizes stability with ${allocation.benqi}% in Benqi lending (${benqiAPY.toFixed(1)}% APY), `
    }

    if (allocation.traderJoe >= 30) {
      rationale += `includes ${allocation.traderJoe}% in TraderJoe LP for balanced risk-reward (${traderJoeAPY.toFixed(1)}% APY), `
    }

    if (allocation.yieldYak > 0) {
      rationale += `and allocates ${allocation.yieldYak}% to YieldYak farming for higher yields (${yieldYakAPY.toFixed(1)}% APY). `
    } else {
  ...
  calculateSurplus(
    ...
    const emergencyFundNeeded = Math.max(0, (monthlyExpenses * emergencyFundTarget) - currentEmergencyFund)
    ...
    if (emergencyFundNeeded > 0) {
      ...
      const emergencyFundAllocation = Math.min(monthlySurplus * 0.5, emergencyFundNeeded)
    ...
    const recommendedInvestmentPercentage = Math.min(80, Math.max(20, 
  ...
  async generateInvestmentRecommendation(
    ...
    try {
      ...
      const allocation = await this.generateAllocationStrategy(userProfile.riskScore, marketData)
      ...
      let description = 'Market conditions present opportunities for yield optimization.'
      let confidence = 0.7
      ...
      if (currentPortfolioValue < config.MIN_PORTFOLIO_VALUE_USD) {
        ...
        description = `Based on your monthly surplus of $${surplusData.investableAmount.toFixed(0)}, consider increasing your DeFi investment allocation.`
        confidence = 0.75
      }

      // Rebalancing recommendation
      const shouldRebalance = await this.shouldRebalance(userProfile, currentPortfolioValue, allocation)
      ...
      logger.error('Failed to generate investment recommendation', error as Error)
  ...
  private async shouldRebalance(
    ...
    if (currentPortfolioValue >= config.MIN_PORTFOLIO_VALUE_USD) {
      return {
        shouldRebalance: true,
        reason: 'Current market yields suggest rebalancing could improve your portfolio performance.',
        confidence: 0.6
      }
    }
    ...
    return {
      ...
      reason: 'Current portfolio allocation is optimal based on your risk profile and market conditions.',
      confidence: 0.7
    }
  }
  ...
  async updateUserPreferencesFromBehavior(
    ...
    const recentActions = actions.filter(
      action => Date.now() - action.timestamp.getTime() < 30 * 24 * 60 * 60 * 1000 // 30 days
    )

    const preferences: Partial<UserProfile['preferences']> = {}

    // Analyze rebalancing frequency
    const rebalanceActions = recentActions.filter(a => a.type === 'rebalance')
    if (rebalanceActions.length > 0) {
      const avgTimeBetweenRebalances = recentActions.length > 1 
        ? recentActions.reduce((sum, action, index) => {
            if (index === 0) return sum
            return sum + (action.timestamp.getTime() - recentActions[index - 1].timestamp.getTime())
          }, 0) / (recentActions.length - 1) / (24 * 60 * 60 * 1000) // Convert to days
        : 30
    ...
    if (successRate < 0.7) {
      preferences.maxSlippage = Math.max(0.5, (preferences.maxSlippage || 2) - 0.5)
    }
