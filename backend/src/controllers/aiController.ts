...
export class AIController {
  ...
  async calculateRiskScore(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const requiredFields: (keyof RiskAssessmentAnswers)[] = ['age', 'income', 'monthlyExpenses', 'investmentGoal', 'riskTolerance', 'investmentExperience']
      const missing = requiredFields.filter(field => answers[field] === undefined || answers[field] === null)
  ...
  async calculateSurplus(req: Request, res: Response, next: NextFunction) {
    try {
      const { monthlyIncome, monthlyExpenses, emergencyFundTarget, currentEmergencyFund } = req.body

      if (typeof monthlyIncome !== 'number' || monthlyIncome < 0) {
        throw new ValidationError('Monthly income must be a non-negative number')
      }

      if (typeof monthlyExpenses !== 'number' || monthlyExpenses < 0) {
        throw new ValidationError('Monthly expenses must be a non-negative number')
      }
      ...
      logger.ai('Surplus calculation requested', surplus.investableAmount / 1000, {
        monthlyIncome,
        monthlyExpenses,
      ...
      const response: ApiResponse = {
        ...
        data: {
          ...
          analysis: {
            ...
            emergencyFundStatus: surplus.emergencyFundNeeded > 0 ? 'needs_funding' : 'adequate',
            recommendation: surplus.investableAmount > 100 
   ...
   */
  async generateRecommendation(req: Request, res: Response, next: NextFunction) {
    try {
      const { address, currentPortfolioValue, monthlyIncome, monthlyExpenses } = req.body
      ...
      if (!userProfile) {
        throw new NotFoundError('User profile not found. Please complete risk assessment first.')
      }
      ...
      logger.ai('Investment recommendation generated', recommendation.confidence, {
    ...
    try {
      const profiles = {
        Conservative: {
          ...
          expectedAPY: '5.5-6.5%',
          suitableFor: ['New investors', 'Risk-averse individuals', 'Short-term goals']
        },
        Balanced: {
          range: '34-66', 
          ...
          expectedAPY: '7.5-9.5%',
          suitableFor: ['Medium-term goals', 'Moderate risk tolerance', 'Diversified approach']
        },
        Aggressive: {
          range: '67-100',
          ...
          expectedAPY: '10-13%',
          suitableFor: ['Long-term goals', 'High risk tolerance', 'Experienced investors']
        }
      }
      ...
      const response: ApiResponse = {
        ...
        data: {
          profiles,
          protocols: {
            benqi: { type: 'Lending', risk: 'Low', description: 'Stable lending yields' },
            traderJoe: { type: 'DEX/LP', risk: 'Medium', description: 'Liquidity provision rewards' },
            yieldYak: { type: 'Farming', risk: 'High', description: 'Auto-compounding strategies' }
          }
        },
        timestamp: new Date()
      }
  ...
  async analyzePortfolio(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const optimalStrategy = await aiService.generateAllocationStrategy(userProfile.riskScore)
      
      // Simple portfolio analysis (in production, this would be more sophisticated)
      const analysis = {
        currentAllocation: portfolioData?.allocation || { benqi: 0, traderJoe: 0, yieldYak: 0 },
        optimalAllocation: {
        ...
        performance: {
          ...
          improvementPotential: optimalStrategy.expectedAPY - (portfolioData?.estimatedAPY || 0)
        },
        recommendations: [
      ...
      logger.ai('Portfolio analysis completed', analysis.performance.improvementPotential, {
        userId: userProfile.id,
